use {
    allnodes_service_protos::{
        client::{self},
        BootstrapInfoRequestPb, BootstrapInfoResponse,
    },
    log::{error, info, warn},
    std::{
...
impl Client {
    ...
    pub async fn get_bootstrap_info(&self) -> Option<BootstrapInfoResponse> {
        let response = self
            ...
            .await
            .inspect_err(|err| error!("Failed to get bootstrap info from Allnodes service: {err}"))
            .ok()?;

        TryFrom::try_from(response.into_inner())
            .inspect_err(|err| {
                error!("Failed to convert bootstrap info from Allnodes service: {err}")
            })
            .ok()
    }
