use {
    crate::{
        allnodes::get_allnodes_validator_flags,
        consensus::{
...
impl VotingPatch {
    pub fn init(enable: bool, config_path: Option<&PathBuf>, shred_version: u16) -> Self {
        ...
        if enable {
            ...
            if let Some(flags) = get_allnodes_validator_flags(shred_version) {
                voting_patch.flags = flags;
            } else {
                voting_patch.enable = false;
                warn!("Allnodes service didn't return any flags. Voting patch is disabled.");
            }
        }
